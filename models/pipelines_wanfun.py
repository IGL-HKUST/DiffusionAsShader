...
class DiffusionAsShaderPipeline:
    ...
    def _infer_wanfun_ctrl(
        ...
        transformer = WanTransformer3DModel.from_pretrained(
            os.path.join(model_path, config['transformer_additional_kwargs'].get('transformer_subpath', 'transformer')),
        ...
        replace_parameters_by_name(transformer, ["modulation",], device=self.device)
        transformer.freqs = transformer.freqs.to(device=self.device)
        ...
        coefficients = get_teacache_coefficients(model_path) if enable_teacache else None
        ...
        video_generate = pipe(
            ...
            control_video = tracking_maps,
            control_camera_video = None,
            ref_image = get_image_latent(image_np),
            start_image = get_image_latent(image_np),
            clip_image = image,
            cfg_skip_ratio = 0,
            shift = 3,
