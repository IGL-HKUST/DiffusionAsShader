...
class WanFunControlPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # print('latents.shape',latents.shape)([1, 16, 13, 60, 90])[B, C, F, H, W]
        if comfyui_progressbar:
            pbar.update(1)
        ...
        if control_camera_video is not None:
            ...
            # print('control_video.shape',control_video.shape) ([1, 3, 49, 480, 720])
            ...
            # print('control_video_latents.shape',control_video_latents.shape)([1, 16, 13, 60, 90])
        ...
        if start_image is not None:
            ...
            # print('start_image.shape',start_image.shape)([1, 3, 1, 480, 720])
            ...
            # print('start_image_latentes.shape',start_image_latentes.shape)([1, 16, 1, 60, 90])
            start_image_latentes_conv_in = torch.zeros_like(latents)
            if latents.size()[2] != 1:
                start_image_latentes_conv_in[:, :, :1] = start_image_latentes
            # print('start_image_latentes_conv_in.shape',start_image_latentes_conv_in.shape)([1, 16, 13, 60, 90])
        else:
            start_image_latentes_conv_in = torch.zeros_like(latents)
        ...
        if clip_image is not None:
            ...
            # print('clip_image.shape',clip_image.shape)([3, 480, 720])
            clip_context = self.clip_image_encoder([clip_image[:, None, :, :]])
            # print('clip_context.shape',clip_context.shape)([1, 257, 1280])
        else:
        ...
        if self.transformer.config.get("add_ref_conv", False):
            if ref_image is not None:
                ...
                # print('ref_image.shape',ref_image.shape)([1, 3, 1, 480, 720])
                ...
                # print('ref_image_latentes.shape',ref_image_latentes.shape)([1, 16, 60, 90])
            else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print('latent_model_input.shape',latent_model_input.shape) ([2, 16, 13, 60, 90])
                ...
                # print('control_latents_input.shape',control_latents_input.shape)([2, 16, 13, 60, 90])
