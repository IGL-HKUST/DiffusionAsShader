...
class DasWanFunInpaintPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        tracking_map: Union[torch.FloatTensor] = None,
        num_frames: int = 49,
        ...
        if tracking_map is not None:
            video_length = tracking_map.shape[2]
            tracking_map = self.image_processor.preprocess(rearrange(tracking_map, "b c f h w -> (b f) c h w"), height=height, width=width) 
            tracking_map = tracking_map.to(dtype=torch.float32)
            tracking_map = rearrange(tracking_map, "(b f) c h w -> b c f h w", f=video_length)
        ...
        if init_video is not None:
            if (mask_video == 255).all():
                ...
                if tracking_map is not None:
                    _, tracking_map_latents = self.prepare_mask_latents(
                        None,
                        tracking_map,
                        batch_size,
                        height,
                ...
                tracking_map_latents_input = torch.cat([tracking_map_latents] * 2) if do_classifier_free_guidance else tracking_map_latents

                print(masked_video_latents_input.shape)
                print(tracking_map_latents_input.shape)
                # Concatenate first channel of mask_latents with last 15 channels of tracking_map_latents
                masked_video_latents_input = torch.cat([masked_video_latents_input[:, :, :1, :, :], tracking_map_latents_input[:, :, -12:, :, :]], dim=2)
                print(masked_video_latents_input.shape)
                y = torch.cat([mask_input, masked_video_latents_input], dim=1).to(device, weight_dtype) 
